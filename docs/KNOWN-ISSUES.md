...
- [x] ~~**`gateway.yaml`'s `public_port`/`admin_port` fields looked load-bearing but did nothing**~~  
  **Found by:** a real deployer's feedback — assumed the port was meant to stay out of `public_base_url`/`admin_base_url` since a dedicated port field existed, got a non-working instance until putting the port directly in the URL instead. Traced to `backend/app/config/deployment.py`'s `InstanceConfig`: `public_port`/`admin_port` were accepted, validated, and threaded through to `TrustAnchor.config_json`/`InstanceSummary`, but every actual connection (`proxy.py`'s `base_url`, `capability_probe.py`, the startup `entity_id` probe) used `str(admin_base_url)` verbatim — the port fields never combined with the URL. The frontend hook typed for the one endpoint that echoed them back (`useInstances.ts` → `GET /api/v1/admin/instances`) was itself dead code, imported nowhere — so there wasn't even a display surface making use of them. Confirmed via `docs/superpowers/specs/2026-04-23-deployment-configuration-design.md`: these were designed as "optional display/diagnostic" fields from the start, a feature that was never wired up.  
  **Fixed:** removed `public_port`/`admin_port` entirely — `InstanceConfig`, `InstanceSummary`, the `TrustAnchor.config_json` payload, `useInstances.ts`'s type, and every `gateway.yaml` example (`backend/config/gateway.yaml`, `README.md`, `docs/DEPLOYMENT.md`, `docs/FEDERATION-TOPOLOGY.md`) — plus an explicit callout in `docs/DEPLOYMENT.md`: put the port directly in `public_base_url`/`admin_base_url`, there is no separate field. Verified live: backend test suite (118 passed) and a full compose stack rebuild + login + trust-anchor list + proxy call, all against the trimmed config.

- [x] ~~**Single-instance deployments still required a manual click to select the only instance**~~  
  **Fixed:** `BackendSwitcher.tsx` now auto-selects when there's exactly one deployment-managed instance and none is active yet — matches real deployer feedback ("if there is only one it should be auto selected"). With several instances, behavior is unchanged.  
  **Found while verifying this fix — severity: high, not cosmetic.** Testing it against a genuinely fresh browser session (real "first ever login") surfaced that every generated-client call (entity configuration, subordinates — most of the app's data) 403'd with `"Not authenticated"` immediately after selecting an instance. Confirmed pre-existing and unrelated to auto-select itself (manual selection on a fresh session reproduces it identically, just less reliably/obviously than auto-select's instant, deterministic trigger). **Root cause:** `AuthProvider`'s mount effect calls `initTokenManager()` (installs `OpenAPI.TOKEN` as a lazy resolver reading `localStorage` fresh per request), then — for a genuinely fresh session with nothing stored yet — calls `clearTokens()` to wipe stale state. `clearTokens()` also unconditionally did `OpenAPI.TOKEN = undefined`, clobbering the resolver *just* installed in the same effect. `login()` never reinstalls it (by design — the resolver is meant to stay live), so `OpenAPI.TOKEN` stayed permanently `undefined` for the rest of that session. A page reload masked it entirely: on reload, storage already holds valid tokens, so the mount effect takes the "restore session" branch and never calls `clearTokens()` at all — meaning this bug was invisible to anyone testing with a warm/already-logged-in browser, which is every normal dev workflow.  
  **Fixed:** `clearTokens()` (`src/lib/token-manager.ts`) no longer touches `OpenAPI.TOKEN` — clearing the underlying storage already makes the resolver correctly resolve to "no token" on its own. Verified against a genuinely fresh Playwright browser context: before, 8 failed "Not authenticated" requests and an unusable Dashboard; after, zero failed requests, Dashboard renders real data (139 entities, 47 active, 31 pending approvals) with no manual click at all.
...
- [x] **Bug 5 (2026-08-12) — `/resolve` returns a valid trust chain for a `blocked` subordinate**  
  ...
  **Evidence, corrected:** originally justified as "ruled out caching by restarting `mesh-ia` first" — that reasoning doesn't hold, see the metadata-policy entry below (`docker compose restart` does not clear LightHouse's persisted state at all, so it proves nothing about caching either way). The real, solid evidence is **static**: `go-oidfed/lib`'s `trustresolver.go` (backs `TrustResolver.ResolveToValidChains()`, what `/resolve` ultimately calls) has zero references to subordinate `status` anywhere in its chain-walking logic — it validates JWKS/signatures and hierarchy (`authority_hints`) but never queries status at all, unlike whatever backs `/list`'s filtering. Per OIDF Federation 1.0 §8.3, resolving a chain through a blocked entity should fail (4xx).  
  **Fixed and verified (2026-08-26).** Gabriel Zachmann (LightHouse): actual root cause was `/fetch` not checking subordinate status at all (not precisely `trustresolver.go` as guessed above, but "goes in the right direction") — added the check there. **Verified live against `oidfed/lighthouse:0.22.3`** (`sha256:828cafdf07ecb933e136d735afd69d05010e24da87191e8d25998bc0dbe52889`, released 2026-08-24): re-ran `mesh-tests/test_subordinate_revocation.py` (renamed `test_blocked_subordinate_no_longer_resolves`, asserting the fixed `404`) — passes. **Adopted as this repo's pinned image** — `docker-compose.yml` now pins `0.22.3`; see the "Upgrading to 0.22.x" note below for the migration step that came with it.

- [x] **Bug 6 (2026-08-15) — Deleting a trust mark type's owner doesn't release its `entity_id` for reuse**  
  Image: `oidfed/lighthouse@sha256:f03d85e86dda38837afec839535a9e1ebbfdab9cd03ec73c53097dbd6aaad223`. Found while building `mesh-tests/test_trust_mark_delegation.py` (`MESH-TESTING-PROGRESS.md` item C4).  
  **Steps to reproduce:** (1) `POST /api/v1/admin/trust-marks/types/1/owner` body `{"entity_id": "https://mesh-tm-owner.example.org", "jwks": {...}}` → 201, created. (2) `DELETE /api/v1/admin/trust-marks/types/1/owner` → 204. (3) Confirm gone everywhere: `GET .../types/1/owner` → 404; `GET /api/v1/admin/trust-marks/owners` (global list) → `[]`; `GET /api/v1/admin/trust-marks/owners/{1..5}` (direct probe) → 404 for all. (4) `POST /api/v1/admin/trust-marks/types/1/owner` again, same `entity_id`, different `jwks`.  
  **Response:** `409 {"error":"invalid_request","error_description":"trust mark owner already exists"}`.  
  **Expected:** 201 — the `entity_id` was confirmed unused by every read path immediately beforehand.  
  **Actual:** 409, immediately, within the same session, not just across restarts. A never-before-used `entity_id` succeeds normally, isolating this precisely to reuse of a previously-deleted `entity_id`.  
  **Root cause (likely):** some internal uniqueness constraint/record isn't cleaned up by `DELETE` — orphaned, invisible to every list/get endpoint but still enforced on create.  
  **Fixed and verified (2026-08-26).** Reported fixed upstream; **verified live against `oidfed/lighthouse:0.22.3`**: `mesh-tests/test_trust_mark_delegation.py`'s full 3-test suite (which already exercises owner create/delete/re-registration as part of its normal flow) passes clean. **Adopted as this repo's pinned image.**

- [x] **Bug 7 (2026-08-15) — Trust Mark Status: an expired mark is reported as `invalid`, not `expired`**  
  Image: `oidfed/lighthouse@sha256:f03d85e86dda38837afec839535a9e1ebbfdab9cd03ec73c53097dbd6aaad223`. Found while building `mesh-tests/test_trust_mark_revocation_expiry.py` (`MESH-TESTING-PROGRESS.md` item C6).  
  **Steps to reproduce:** (1) `PATCH /api/v1/admin/trust-marks/issuance-spec/1` body `{"lifetime": 2}` (seconds). (2) `GET /trust_mark?sub=...&trust_mark_type=...` → 200, fresh validly-signed mark, `exp` ~2s out. (3) Wait past `exp`. (4) `POST /trust_mark/status` (`application/x-www-form-urlencoded`, `trust_mark=<jwt>`).  
  **Response:** `{"status": "invalid"}`.  
  **Expected:** `{"status": "expired"}` — per OIDF Federation 1.0 §8.4.2, `expired` and `invalid` are distinct, defined values.  
  **Root cause, confirmed (2026-08-26, upstream):** Gabriel Zachmann (LightHouse): JWT verification checks signature and expiration together in one pass, so an expired-but-otherwise-valid mark fails that combined check and gets classified as generic `invalid` before any separate, later expiration-specific status logic ever runs. No fix was explicitly reported for this one (only Bugs 5/6/8 were).  
  **Fixed anyway, verified (2026-08-26).** **Verified live against `oidfed/lighthouse:0.22.3`**: re-ran `mesh-tests/test_trust_mark_revocation_expiry.py` (renamed `test_expired_mark_status_is_expired`, asserting the fixed `"expired"`) — passes — likely an incidental fix from the same JWT-verification work that fixed Bug 5, not a targeted one. **Adopted as this repo's pinned image.**

- [x] **Bug 8 (2026-08-15) — `PUT /subordinates/{id}/constraints` silently freezes that subordinate's metadata policy, decoupling it from future general-policy changes**  
  Image: `oidfed/lighthouse@sha256:f03d85e86dda38837afec839535a9e1ebbfdab9cd03ec73c53097dbd6aaad223`. Found while fixing `mesh-tests/test_metadata_policy.py` (`MESH-TESTING-PROGRESS.md` item B1) after it started failing depending on which other tests had run first in the same session.  
  **Steps to reproduce (isolated with a disposable subordinate):** (1) `POST /api/v1/admin/subordinates` → 201; confirmed via direct `sqlite3` inspection of `lighthouse.db` that its `metadata_policy` column is genuinely `NULL`. (2) `PUT /api/v1/admin/subordinates/metadata-policies/federation_entity/organization_name` body `{"value": "Live Follow Test"}`. (3) `GET /fetch?sub=<subordinate>` → correctly shows `"Live Follow Test"` — live-follows the general policy. (4) Change the general policy again, confirm it still live-follows. (5) `PUT /api/v1/admin/subordinates/{id}/constraints` body `{"allowed_entity_types": ["openid_provider"]}`, then `DELETE` the same. (6) Change the general policy to a third value. (7) `GET /fetch?sub=<subordinate>` again.  
  **Response:** `metadata_policy` still shows the value from step 2/4, not step 6.  
  **Expected:** should keep live-following the general policy, as in steps 3-4.  
  **Actual:** frozen at whatever the general policy evaluated to the instant the `constraints` `PUT` in step 5 ran — confirmed via direct SQL inspection, the column flipped from `NULL` to a materialized JSON snapshot exactly then, not before.  
  **Root cause (likely):** the `constraints` update handler probably does a full-row save/upsert on the subordinate record that includes `metadata_policy`, populated from whatever the general policy currently resolves to — a full-object-write side effect.  
  **Fixed and verified (2026-08-26).** Reported fixed upstream; **verified live against `oidfed/lighthouse:0.22.3`**: `mesh-tests/test_metadata_policy.py`'s full 2-test suite passes clean (steps 1-7 above all behave correctly — no freeze). **Adopted as this repo's pinned image** — `mesh-leaf-rp` in the live demo stack may still carry an already-frozen `metadata_policy` from before the upgrade (harmless; whether the fix unfreezes existing frozen records automatically or needs the documented `metadata-policies` re-sync call is unconfirmed — revisit if it comes up).

- **Upgrading to 0.22.x is not a drop-in image swap.** Discovered while verifying Bugs 5-8 above: LightHouse 0.22.x moved federation endpoint configuration (`endpoints:`, and several other sections — signing algorithm, key rotation, metadata policies, trust anchors, trust mark specs) from being read live from `config.yaml` to being read from the database, seeded once via the `lhmigrate config2db` tool now shipped in the image (`lhmigrate` was already present in earlier images too, just not previously needed). Confirmed empirically: on a fresh 0.22.3 container using our existing (schema-valid, `lhmigrate config` reports zero changes needed) `config.yaml`, **every public federation endpoint returns 404** — including `/fetch`, `/list`, `/resolve`, `/trust_mark` — until `lhmigrate config2db --config=config.yaml --db-dir=<data dir>` is run once and the container restarted; the admin API is unaffected either way (it's still read live from `config.yaml`). Nowhere documented that we could find. **Resolved:** `scripts/migrate-lighthouse-config.py` (new) runs `lhmigrate config2db` for every instance and restarts it, mirroring `bootstrap-lighthouse-admin-users.py`'s one-time-setup pattern — idempotent, safe to re-run. Wired into the documented setup sequence (`CLAUDE.md` hard constraint #13, `docs/GETTING-STARTED.md`, `docs/FEDERATION-TOPOLOGY.md`). `docker-compose.yml` now pins `0.22.3`.
