...
- [x] **Bug 5 (2026-08-12) — `/resolve` returns a valid trust chain for a `blocked` subordinate**  
  ...
  **Fixed and verified (2026-08-26).** Gabriel Zachmann (LightHouse): actual root cause was `/fetch` not checking subordinate status at all (not precisely `trustresolver.go` as guessed above, but "goes in the right direction") — added the check there. **Verified live against `oidfed/lighthouse:0.22.3`** (`sha256:828cafdf07ecb933e136d735afd69d05010e24da87191e8d25998bc0dbe52889`, released 2026-08-24): re-ran `mesh-tests/test_subordinate_revocation.py::test_blocked_subordinate_still_resolves_TODO_LIGHTHOUSE_BUG` (a test deliberately written to assert the *buggy* HTTP 200) — now correctly gets `404`, in an isolated worktree, not this repo's actual mesh-tests file — flipping that test to assert the fixed behavior happens together with actually adopting 0.22.x as the pin (still asserting the *buggy* 200 today, correctly, since our pinned image still has the bug). **Not yet adopted as this repo's pinned image** — see the new "Upgrading to 0.22.x" note below for why.
...
- [x] **Bug 6 (2026-08-15) — Deleting a trust mark type's owner doesn't release its `entity_id` for reuse**  
  ...
  **Fixed and verified (2026-08-26).** Reported fixed upstream; **verified live against `oidfed/lighthouse:0.22.3`**: `mesh-tests/test_trust_mark_delegation.py`'s full 3-test suite (which already exercises owner create/delete/re-registration as part of its normal flow) passes clean. Not yet adopted as this repo's pinned image.
...
- [x] **Bug 7 (2026-08-15) — Trust Mark Status: an expired mark is reported as `invalid`, not `expired`**  
  ...
  **Fixed anyway, verified (2026-08-26).** **Verified live against `oidfed/lighthouse:0.22.3`**: re-ran `mesh-tests/test_trust_mark_revocation_expiry.py::test_expired_mark_status_TODO_LIGHTHOUSE_BUG` (deliberately written to assert the *buggy* `"invalid"` status) — now correctly gets `"expired"`, in an isolated worktree — likely an incidental fix from the same JWT-verification work that fixed Bug 5, not a targeted one. Same as Bug 5: this repo's actual test file still (correctly) asserts the buggy `"invalid"` until 0.22.x is adopted as the pin. Not yet adopted as this repo's pinned image.
...
- [x] **Bug 8 (2026-08-15) — `PUT /subordinates/{id}/constraints` silently freezes that subordinate's metadata policy, decoupling it from future general-policy changes**  
  ...
  **Fixed and verified (2026-08-26).** Reported fixed upstream; **verified live against `oidfed/lighthouse:0.22.3`**: `mesh-tests/test_metadata_policy.py`'s full 2-test suite passes clean (steps 1-7 above all behave correctly — no freeze). `mesh-leaf-rp` in this repo's own mesh is still in its already-frozen state from the original investigation (harmless, and untouched by this verification pass since it ran in an isolated worktree, not against the live repo's data) — whether the fix unfreezes existing frozen records automatically or needs the documented `metadata-policies` re-sync call is unconfirmed, revisit once 0.22.x is actually adopted. Not yet adopted as this repo's pinned image.

- **Upgrading to 0.22.x is not a drop-in image swap.** Discovered while verifying Bugs 5-8 above: LightHouse 0.22.x moved federation endpoint configuration (`endpoints:`, and several other sections — signing algorithm, key rotation, metadata policies, trust anchors, trust mark specs) from being read live from `config.yaml` to being read from the database, seeded once via the `lhmigrate config2db` tool now shipped in the image (`lhmigrate` was already present in earlier images too, just not previously needed). Confirmed empirically: on a fresh 0.22.3 container using our existing (schema-valid, `lhmigrate config` reports zero changes needed) `config.yaml`, **every public federation endpoint returns 404** — including `/fetch`, `/list`, `/resolve`, `/trust_mark` — until `lhmigrate config2db --config=config.yaml --db-dir=<data dir>` is run once and the container restarted; the admin API is unaffected either way (it's still read live from `config.yaml`). Nowhere documented that we could find. This repo's `bootstrap-lighthouse-admin-users.py`-style one-time-setup pattern would need a sibling step (or an extension to that script) before adopting a `0.22.x`+ pin for real — not built yet, tracked as a prerequisite for updating `docker-compose.yml`'s pinned digest.
