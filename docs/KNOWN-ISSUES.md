...
- [ ] **Bug 5 (2026-08-12) — `/resolve` returns a valid trust chain for a `blocked` subordinate**  
...
- [ ] **Bug 6 (2026-08-15) — Deleting a trust mark type's owner doesn't release its `entity_id` for reuse**  
  ...
  **Root cause (likely):** some internal uniqueness constraint/record isn't cleaned up by `DELETE` — orphaned, invisible to every list/get endpoint but still enforced on create. Minor (real deployments would rarely reuse a deleted owner's exact identifier), but real. Worked around in the test suite with a fresh `entity_id` per test run. Not filed upstream yet.

- [ ] **Bug 7 (2026-08-15) — Trust Mark Status: an expired mark is reported as `invalid`, not `expired`**  
  ...
  **Expected:** `{"status": "expired"}` — per OIDF Federation 1.0 §8.4.2 (normative text, not recalled from memory), `expired` ("the Trust Mark has expired") and `invalid` ("signature validation failed or another error was detected") are distinct, defined values.  
  **Actual:** a real, validly signed mark whose only problem is `exp` has passed is classified the same as a mark with a bad signature or malformed structure.  
  **Root cause (likely):** the expiry check isn't returning its own distinct status — falls through to whatever generic "couldn't validate" path handles signature/structural failures, instead of checking `exp` first.  
  **Note:** revocation is unaffected and works correctly — blocking a `TrustMarkSubject` (`PUT .../subjects/{id}/status`) correctly flips an already-issued mark's status check to `"revoked"`, and blocks fresh issuance with 403 — isolating this to the expiry path specifically. Lower priority than Bug 5 (a relying party still correctly treats the mark as no good, just for the wrong stated reason). Not filed upstream yet.

- [ ] **Bug 8 (2026-08-15) — `PUT /subordinates/{id}/constraints` silently freezes that subordinate's metadata policy, decoupling it from future general-policy changes**  
  ...
  **Actual:** frozen at whatever the general policy evaluated to the instant the `constraints` `PUT` in step 5 ran — confirmed via direct SQL inspection, the column flipped from `NULL` to a materialized JSON snapshot exactly then, not before. Toggling `status` alone never triggers it; a `constraints` `PUT` triggers it every time, immediately. **The freeze survives `DELETE`ing the constraint again and survives `docker compose restart`** — persisted in SQLite, not an in-process cache.  
  **Recovery:** `POST /api/v1/admin/subordinates/{id}/metadata-policies` ("copy general metadata policies to subordinate") re-syncs it — the intended explicit-sync mechanism (mirrors `copyGeneralConstraintsToSubordinate`'s pattern), so the bug is specifically that an unrelated `constraints` write silently triggers this freeze as an undocumented side effect.  
  **Root cause (likely):** the `constraints` update handler probably does a full-row save/upsert on the subordinate record that includes `metadata_policy`, populated from whatever the general policy currently resolves to — a full-object-write side effect. `mesh-leaf-rp` in this repo's own mesh is permanently in the frozen state as a result of this investigation (harmless). Not filed upstream yet.
