...
test.describe('Navigation labels @proxy', () => {
  test('sidebar uses spec-aligned subordinate terminology', async ({ instancePage: page }) => {
    ...
    const subordinatesTrigger = page.getByRole('button', { name: 'Subordinates', exact: true });
    await expect(subordinatesTrigger).toBeVisible();
    // Subordinates starts collapsed (matches the icon rail, which never shows
    // children inline) — expand it to check the child link labels.
    await subordinatesTrigger.click();
